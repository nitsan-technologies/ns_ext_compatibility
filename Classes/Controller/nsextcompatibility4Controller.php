 ...
 *
 */
class Tx_NsExtCompatibility_Controller_nsextcompatibility4Controller extends Tx_Extbase_MVC_Controller_ActionController {
    ...
    */
    ...
    {   
        ...
        $filename = strtolower(trim(preg_replace('#\W+#', '_', $sysDetail['sitename']), '_'));
        ...
        $excel->getActiveSheet()->getStyle('A11:N11')->applyFromArray($extTableHeadingStype);
        ...
        $extTableHeader->setCellValue("D11",$this->translate('action'));
         $extTableHeader->setCellValue("E11",$this->translate('compatible4'));
        $extTableHeader->setCellValue("F11",$this->translate('compatible6'));
        $extTableHeader->setCellValue("G11",$this->translate('compatible7'));
        $extTableHeader->setCellValue("H11",$this->translate('compatible8'));
        $extTableHeader->setCellValue("I11",$this->translate('compatible9'));
        $extTableHeader->setCellValue("J11",$this->translate('currentVersion'));
        $extTableHeader->setCellValue("K11",$this->translate('newVersion'));
        $extTableHeader->setCellValue("L11",$this->translate('state'));
        $extTableHeader->setCellValue("M11",$this->translate('type'));
        $extTableHeader->setCellValue("N11",$this->translate('notes'));
       
      
        //Hide Compatibility columns start
        if($sysDetail['typo3version']<6 && $sysDetail['targetVersion']>=4){
        ...
        if($sysDetail['typo3version']<9 && $sysDetail['targetVersion']>=8){
          ...
          if($sysDetail['typo3version']<10 && $sysDetail['targetVersion']>=9){
            $excel->getActiveSheet()->getColumnDimension('I')->setVisible(true);
        }else{
       ...
       //Add Data in Excel From array $extensionlist end
        ...
        foreach ($extensionlist as $key=> $extension) {
            if($extension['key']!='ns_ext_compatibility'){
                ...
                $row->setCellValue("C$i",$extension['extkey']);
                ...
                if($extension['compatible9']==1){
                    $row->setCellValue("I$i",$this->translate('yes'));
                    $excel->getActiveSheet()->getStyle("I$i")->applyFromArray($syleYes);
                }else{
                    $row->setCellValue("I$i",$this->translate('no'));
                    $excel->getActiveSheet()->getStyle("H$i")->applyFromArray($syleNo);
                }
                
                $row->setCellValue("J$i",$extension['version']);
                if($extension['newVersion']>$extension['version']){
                    $row->setCellValue("K$i",$extension['newVersion']);
                    $excel->getActiveSheet()->getStyle("K$i")->applyFromArray($syleYes);
                }else{
                    if($extension['updateToVersion']['version']>$extension['version']){
                        $row->setCellValue("K$i",$extension['updateToVersion']['version']);
                        $excel->getActiveSheet()->getStyle("K$i")->applyFromArray($syleYes);
                    }else{
                        $row->setCellValue("K$i",$extension['version']);
                    }
                }
                $row->setCellValue("L$i",$extension['state']);
                if($extension['state']=='Stable'){
                    $excel->getActiveSheet()->getStyle("L$i")->applyFromArray($syleYes);
                }elseif ($extension['state']=='Beta') {
                    $excel->getActiveSheet()->getStyle("L$i")->applyFromArray($syleBeta);
                }elseif ($extension['state']=='Alpha') {
                    $excel->getActiveSheet()->getStyle("L$i")->applyFromArray($syleNo);
                }
                if($extension['updateToVersion']!=null){
                    $row->setCellValue("M$i",$this->translate('ter'));
                }else{
                   $row->setCellValue("M$i",$this->translate('custom'));
                }
                $k++;
                $i++;
            }
        ...
        $row->setCellValue("E$i",$overviewReport['totalCompatible4']);
        $row->setCellValue("F$i",$overviewReport['totalCompatible6']);
        $row->setCellValue("G$i",$overviewReport['totalCompatible7']);
        $row->setCellValue("H$i",$overviewReport['totalCompatible8']);
        $row->setCellValue("I$i",$overviewReport['totalCompatible9']);
        ...
        $excel->getActiveSheet()->getStyle("A$i:N$i")->applyFromArray($overviewReportStyle);
